...
gsigs = 'https://github.com/amerocrypto/gitian.sigs.git'
gbrepo = 'https://github.com/amerocrypto/gitian-builder.git'
...
def setup():
    ...
    if not os.path.isdir('amero'):
        subprocess.check_call(['git', 'clone', args.url, 'amero'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/amero/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'amero='+args.commit, '--url', 'amero='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/amero-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/amero/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/amero/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/amero-project/amero', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/amero', exist_ok=True)
    os.chdir('builder/inputs/amero')
    if args.pull:
