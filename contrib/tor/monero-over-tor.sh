...
if test -z "$amerod"
then
  echo "amerod not found"
  exit 1
fi
echo "Found: $amerod"

TORDIR="$DIR/amero-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting amerod"
  tail -n 400 "$HOME/.amero/amero.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Amero\ \'" | tail -n 20
  exit 1
fi
